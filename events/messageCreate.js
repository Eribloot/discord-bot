...
if(testingOn)
{
  module.exports = {
    ...
    async execute(message)
    {
      ...
      // get info
      const currentUser = await Users.findOne({where: { user_id: message.author.id }})
      ...
      if(currentUser)
      {
        await Users.update({ message_time: messageCreated }, { where: { user_id: message.author.id }});
        return console.log("Most recent message updated.");
      }

        //if user doesn't exist
      else
        return await Users.create({ username: currentName, user_id: message.author.id, message_time: messageCreated });
